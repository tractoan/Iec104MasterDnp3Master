            ...
            if (iec104MasterDnp3Master->pendingCommands.counter.size() > 0){
                ...
                if (sameAsdu){
                    ...
                    if (iec104MasterDnp3Master->pendingCommands.counter.size() > 0){
                        ...
                        for (int i=0 ; i<iec104MasterDnp3Master->pendingCommands.counter.size() ; i++) {
                            BinaryCounterReading binaryCounter = BinaryCounterReading_create(NULL, iec104MasterDnp3Master->pendingCommands.counter[i].counterValue, 0, true, true, true);
    ...
    if (CS101_ASDU_getTypeID(asdu) == C_SC_NA_1) {
        if  (CS101_ASDU_getCOT(asdu) == CS101_COT_ACTIVATION) {
            ...
            if (io) {
                ...
                std::cout << "ERROR: message has no valid information object\n";
                return false;
            }
        }
        else {
...
bool Iec104MasterDnp3Master::processingCommand(){
    ...
    for (int i=0 ; i<pendingCommands.analogInput.size() ; i++){
        dnp3Master->ScanRange(GroupVariationID(30, 1), pendingCommands.analogInput[i].dnp3Point, pendingCommands.analogInput[i].dnp3Point, dnp3IsoeHandler);
    }
    for (int i=0 ; i<pendingCommands.counter.size() ; i++){
        dnp3Master->ScanRange(GroupVariationID(20, 1), pendingCommands.counter[i].dnp3Point, pendingCommands.counter[i].dnp3Point, dnp3IsoeHandler);
    }
        ...
        {		
            ...
            auto checkErr = [&](const CommandPointResult& res)
            {
                if (res.status == CommandStatus::SUCCESS){
                    ...
                    pendingCommands.binaryOutput[i].state = Iec104MasterDnp3MasterMessageConfig::Iec104MasterDnp3MasterMessageState::RESPONSE_SUCCESS;
                } else {
                    std::cout << "binary output failed" << std::endl;
                    pendingCommands.binaryOutput[i].state = Iec104MasterDnp3MasterMessageConfig::Iec104MasterDnp3MasterMessageState::RESPONSE_FAILED;
                }
            };
    ...
    for (int i=0 ; i<pendingCommands.analogOutput.size() ; i++){
        ...
        {			
            ...
            auto checkErr = [&](const CommandPointResult& res)
            {
                if (res.status == CommandStatus::SUCCESS){
                    std::cout << "binary output success" << std::endl;
                    pendingCommands.analogOutput[i].state = Iec104MasterDnp3MasterMessageConfig::Iec104MasterDnp3MasterMessageState::RESPONSE_SUCCESS;
                } else {
                    std::cout << "binary output failed" << std::endl;
                    pendingCommands.analogOutput[i].state = Iec104MasterDnp3MasterMessageConfig::Iec104MasterDnp3MasterMessageState::RESPONSE_FAILED;
                }
            };
            result.ForeachItem(checkErr);
